        ...
        <div class="proListTable">
            <el-table :data="proData" style="width: 100%" border>
                ...
                <el-table-column label="快递单号">
                    <template slot-scope="scope">{{scope.row.OrderCode?scope.row.OrderCode:'无'}}
                        <el-button type="info" class="tag-read" :data-clipboard-text="scope.row.OrderCode" plain
                            @click="copyOrder(scope.row)" size="mini">复制</el-button>
                    </template>
                </el-table-column>
                <el-table-column label="打单时间">
                    <template
                        slot-scope="scope">{{scope.row.logisticsTime?tranTime(scope.row.logisticsTime):'无'}}</template>
                </el-table-column>
                ...
                <el-table-column label="操作">
                    <template slot-scope="scope">
                        <el-button size="mini" @click="lookExpressNumber(scope.row)" type="primary"
                            v-if="scope.row.OrderCode">查看单号</el-button>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            lookExpressNumber(row) {
                req.post('/api/logistics/getOddImg', {
                    order: row.order
                }).then(res => {
            ...
            getPro() {
                req
                    .post("/api/placeOrder/getmyOrderList", {
